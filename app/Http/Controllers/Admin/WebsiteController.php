...
class WebsiteController extends Controller
{
    ...
    public function index()
    {
        ...
        $title = "Websites";
        $websites = DB::table('websites')->where('default_site', 1)->get();

        return view('admin.websites.index', compact('websites', 'title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($isReportCreated) {
            ...
            /*--- RegularPrices*/
            $terminals_without_website = RegularPrices::where('website_id', '!=', $Savedwebsite_id)->whereIn('service_id',array('1','3'))->orderBy('website_id', 'asc')->groupBy('terminal_id', 'service_id')->get();
            if (!empty($terminals_without_website)) {
                foreach ($terminals_without_website as $tww) {
                    $found = RegularPrices::where('terminal_id', '=', $tww->terminal_id)->where('service_id', '=', $tww->service_id)->firstOrFail();
                    if ($found) {
            ...
            if (!empty($reg_disc_terminals_without_website)) {
                foreach ($reg_disc_terminals_without_website as $tww) {
                    $found = RegularDiscounts::where('terminal_id', '=', $tww->terminal_id)->firstOrFail();
                    if ($found) {
                        $replicated = $found->replicate();
                        $replicated->website_id = $Savedwebsite_id;
                        $replicated->save();
    ...
    public function edit($id)
    {
        ...
        return view('admin.websites.edit', compact('website', 'id'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect('/admin/websites')->with('success', 'website has been updated');
