            ...
            foreach ($allwebsites as  $c_website) {
                foreach ($allservices as $service) {
                    foreach ($allterminals as $key => $terminal) {
                        ...
                        $not_working_hours_price = Domain::GetNotWorkingHoursPrice($c_website->id,  $bk_date1, $bk_date2);
       ...
       foreach($prepared_session_data['vehicles'] as $key=>$vehical){
        ...
        if ($err == 0 && !empty($payment_option)) {
            foreach($prepared_session_data['vehicles'] as $key=>$vehical){
                ...
                if ($booking_update->bk_discount_offer_coupon <> "") {
                    $TOTAL_PAYABLE_AMOUNT = $booking_update->bk_final_amount + $carwash + $booking_update->not_working_hours + $booking_update->last_min_booking +  $booking_update->charging_service_charges + $booking_update->charging;
                } else {
                ...
                if (!empty($booking_exists)) { // booking exists
                        ...
                        if (!empty($booking_exists_for_customer)) { // booking exists for customer
                            ...
                            if ($payment_option == 1) {
                                ...
                                $querystring .= "amount=$all_vehical_final_total&";
                                $querystring .= "desc=$booking_update->refrence_num_common&";
                                ...
                                try {
                                    $charge = \Stripe\Charge::create([
                                        ...
                                        'description' => $booking_update->refrence_num_common,
                                    ]);
                                    ...
                                    if ($charge['status'] == 'succeeded') {
                                        ...
                                        if ($txn_idexists) {
                                            ...
                                            $PaymentNotification = new PaymentNotification([
                                                ...
                                                'txn_id' => $charge['id'],
                                                'item_name' => $booking_update->refrence_num_common,
                                                'log' => serialize($charge)
                                            ]);
                                            $PaymentNotification->save();
                                            $insert_id = $PaymentNotification->id;
                                            if ($insert_id) {
                                                // send notification to all vehicals emails
                                                foreach ($saved_booking_id as $booking_id) {
                                                    $bookingdata = $this->GetBookingById($booking_id);
        ...
        if( !empty($prepared_session_data) && count($prepared_session_data) > 5){
            ...
            //dd($errors);
                ...
                if(empty($errors)){
                    ...
                    return view($domain->website_templete . '.booking', compact('page_title', 'meta_array', 'services', 'domain', 'html', 'html_price_only', 'colors', 'settings', 'customer', 'show_login', 'prepared_session_data', 'skey', 'vehicaltype', 'carwash_selected', 'totalBookingPrice'))->withErrors($errors);
                }
        }else{
